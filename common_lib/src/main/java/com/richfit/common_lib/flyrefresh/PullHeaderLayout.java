...
import com.richfit.common_lib.flyrefresh.internal.ElasticOutInterpolator;
import com.richfit.common_lib.flyrefresh.internal.SimpleAnimatorListener;
import com.richfit.common_lib.utils.UiUtil;

/**
 * Created by monday on 15/5/18.
 */
public class PullHeaderLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    final static int ACTION_BUTTON_CENTER = UiUtil.dpToPx(40);
    final static int ACTION_ICON_SIZE = UiUtil.dpToPx(32);
    private final static int DEFAULT_EXPAND = UiUtil.dpToPx(300);
    private final static int DEFAULT_HEIGHT = UiUtil.dpToPx(240);
    private final static int DEFAULT_SHRINK = UiUtil.dpToPx(48);
    ...
    public void setActionDrawable(Drawable actionDrawable) {
        ...
        if (mActionDrawable != null) {
            if (mActionView == null) {
                final int bgColor = UiUtil.getThemeColorFromAttrOrRes(getContext(), R.attr.colorAccent, R.color.colorAccent);
                final int pressedColor = UiUtil.darkerColor(bgColor, 0.8f);
